...
import ProductSpendingView from './product_spending_view';
import ProductService from '../api/product_service';

class ProductSpending extends Component {
  constructor(props) {
    super(props);
    const productId = this.props.match ? parseInt(this.props.match.params.id, 0) : null;
    this.state = {
      chartData: null,
      currentPage: 1,
      desc: true,
  ...
  async loadChart(args) {
    if (this.props.isAuthenticated()) {
      ...
      const response = await ProductService.getChart(params);

...
const ProductsTableRow = withRouter(({ ...props }) => {
  const { id: productId, name, highest_price, history, lowest_price, most_recently_puchased, total_purchased } = props;
  const userId = TokenHelper.userId('jwt');
  const updateProductStateAndLink = async (e) => {
    e.preventDefault();
    await props.loadSpendingTable({ productId, userId, resourceName: 'product', desc: true });
